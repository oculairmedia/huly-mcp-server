...
export const definition = {
  ...
  description:
    'Perform high-efficiency bulk modifications across multiple issues with sophisticated batch processing, comprehensive validation, and detailed progress monitoring for large-scale issue management and workflow automation scenarios. This advanced tool supports simultaneous updates to up to 1000 issues per operation, enabling mass changes to critical fields including title, description, status, priority, component assignment, and milestone association with intelligent batching and performance optimization. The bulk update process provides flexible input format support with case-insensitive status and priority values, comprehensive field validation with relationship verification, and atomic transaction management that ensures data consistency throughout complex multi-issue operations. Advanced features include dry-run validation mode for pre-execution verification, continue-on-error processing for resilient operations, configurable batch sizes for performance tuning, and real-time progress callbacks with detailed success/failure tracking. The tool automatically handles permission validation, dependency checking, and notification triggers while providing comprehensive audit trails and detailed reporting of all modifications, making it essential for sprint transitions, project reorganization, workflow automation, data migration scenarios, and administrative maintenance tasks requiring reliable, scalable, and auditable bulk issue modification capabilities across diverse organizational contexts.',
  inputSchema: {
    type: 'object',
    properties: {
      updates: {
