...
export const definition = {
  ...
  description: 'Execute sophisticated search and filtering operations across issues with advanced query capabilities, multi-dimensional filtering options, and flexible result customization for comprehensive issue discovery and analysis. This powerful search tool supports full-text queries across issue titles and descriptions, precise filtering by status, priority, assignee, component, and milestone parameters, and temporal filtering using creation and modification date ranges with ISO 8601 format support. The search engine provides cross-project capabilities for workspace-wide issue discovery, intelligent result ranking based on relevance and recency, and configurable result limits for performance optimization. Advanced filtering includes support for complex boolean operations, wildcard matching, and nested query structures that enable sophisticated issue analytics and reporting scenarios. The tool automatically handles user permission filtering, ensuring secure access to authorized issues while providing comprehensive result metadata including match scores, highlighting, and contextual information. This tool is indispensable for project managers conducting issue analysis, developers searching for related work items, support teams tracking bug patterns, and stakeholders requiring detailed project insights through flexible, high-performance issue discovery and filtering capabilities.',
  inputSchema: {
    type: 'object',
    properties: {
      project_identifier: {
