...
export const definition = {
  ...
  description:
    'Generate comprehensive issue structures from predefined templates with automatic hierarchical creation, intelligent default value application, and flexible customization options that streamline complex workflow initialization and maintain organizational consistency. This powerful template instantiation tool creates complete issue hierarchies including parent issues and all associated child issues based on template configurations, with intelligent inheritance of default values, automatic relationship establishment, and comprehensive validation throughout the creation process. The template-based creation process supports flexible override capabilities for individual issue properties including title, description, priority, assignee, component, milestone, and estimation values, while maintaining template structure integrity and organizational workflow patterns. Advanced features include configurable child issue inclusion for selective hierarchy creation, automatic validation of template existence and user permissions, comprehensive error handling with detailed failure analysis, and atomic transaction management that ensures complete hierarchy creation or rollback on failure. The tool provides essential capabilities for standardized workflow initialization, complex project setup, team onboarding processes, and organizational scaling through consistent, repeatable issue creation patterns that maintain quality and structure across diverse project types and development methodologies.',
  inputSchema: {
    type: 'object',
    properties: {
      template_id: {
